merge_cecal_serum_species_files <- function(
  input_dir,
  output_dir,
  metabolite_col,
  species_col,
  output_xlsx = "control_cecal_serum_species_merged.xlsx"
) {
  ...
  cecal_files <- list.files(
    input_dir,
    pattern = "^control_cecal_.*_merged.tsv",
    full.names = TRUE,
    recursive = TRUE
  )
  ...
  serum_files <- list.files(
    input_dir,
    pattern = "^control_serum_.*_merged.tsv",
    full.names = TRUE,
    recursive = TRUE
  )
  ...
  species_files <- list.files(
    input_dir,
    pattern = "^control_species_.*_merged.tsv",
    full.names = TRUE,
    recursive = TRUE
  )

  if (length(cecal_files) < 2) stop("Need at least two cecal files to merge.")
  if (length(serum_files) < 2) stop("Need at least two serum files to merge.")
  if (length(species_files) < 2) stop("Need at least two species files to merge.")
...
merge_cecal_serum_species_files(
